...
function listarMarcacaoCliente($cliente, $empresa) {
    global $wpdb;
    $result  = $wpdb->get_results("SELECT * FROM marcacao WHERE cpfcli = '$cliente' AND "
            . "cnpjemp = '$empresa' ORDER BY datamarcacao desc ");
    return $result;
}
...
{
    global $wpdb;
    $result  = $wpdb->get_results("SELECT max(datamarcacao) dtultmarcacao FROM marcacao "
            . "WHERE cpfcli = '$cpf' AND tipomarcacao = 'cash' ");
    return $result;
}
...
function marcarCliente($arrayLigacao, $arrayMarcacao ) {
    ...
    $inserirMarcacao  = $wpdb->insert( "$tabela2", $arrayMarcacao, array('%s','%s','%s','%f','%s', '%s','%f','%f', '%s') );
    if ($inserirMarcacao) {
        return true;
    } else {
        return false;
